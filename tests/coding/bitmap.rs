...
impl Good {
    ...
    pub const fn all() -> &'static [Self] {
        &[
            Self { encoded: &[0b0000_0000], decoded: [false; 8] },
            Self { encoded: &[0b1010_1010], decoded: [true, false, true, false, true, false, true, false] },
            Self { encoded: &[0b0101_0101], decoded: [false, true, false, true, false, true, false, true] },
            Self { encoded: &[0b1111_1111], decoded: [true; 8] },
        ]
    }
}
...
pub fn decode() {
    for test_vector in Good::all() {
        ...
        let decoded = Decoder::new(encoded).bitmap().expect("Failed to decode valid flags");
...
pub fn encode() {
    for test_vector in Good::all() {
        // Encode length
        let length: usize = Length::new().bitmap(&test_vector.decoded).into();

        // Encode and validate
        let encoded = Encoder::default().bitmap(test_vector.decoded);
...
pub fn decode_invalid() {
    for test_vector in BadEncoded::all() {
        // Decode and validate
        let encoded = test_vector.encoded.iter().copied();
        let decoded = Decoder::new(encoded).bitmap();
