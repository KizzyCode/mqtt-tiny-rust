...
use core::ops::Deref;
use mqtt_tiny::{packets::TryFromIterator, Pingreq};
...
impl Good {
    ...
    pub const fn all() -> [Self; 1] {
        [
            // The PINGREQ packet does not carry any configurable data
            Self { encoded: b"\xC0\x00", decoded: Pingreq::new() },
        ]
    }
}
...
pub fn decode() {
    for test_vector in Good::all() {
        ...
        let decoded = Pingreq::try_from_iter(encoded).expect("Failed to decode valid packet");
...
pub fn decode_invalid() {
    for test_vector in BadEncoded::all() {
        // Decode and validate
        let encoded = test_vector.encoded.iter().copied();
        let decoded = Pingreq::try_from_iter(encoded);
