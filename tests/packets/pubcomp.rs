use core::ops::Deref;
use mqtt_tiny::{packets::TryFromIterator, Pubcomp};
...
impl Good {
    ...
    pub const fn all() -> [Self; 1] {
        [
            // The PUBCOMP packet does not have any context specific encoding
            Self { encoded: b"\x70\x02\x04\x07", decoded: Pubcomp::new(0x0407) },
        ]
    }
}
...
impl BadEncoded {
    ...
    pub const fn all() -> &'static [Self] {
        &[
            ...
            Self { encoded: b"\x60\x02\x04\x07" },
            // Packet with invalid length
            Self { encoded: b"\x70\x00" },
        ]
    }
}
...
pub fn decode() {
    for test_vector in Good::all() {
        ...
        let decoded = Pubcomp::try_from_iter(encoded).expect("Failed to decode valid packet");
...
pub fn decode_invalid() {
    for test_vector in BadEncoded::all() {
        // Decode and validate
        let encoded = test_vector.encoded.iter().copied();
        let decoded = Pubcomp::try_from_iter(encoded);
