use core::ops::Deref;
use mqtt_tiny::{packets::TryFromIterator, Unsuback};
...
impl Good {
    ...
    pub const fn all() -> [Self; 1] {
        [
            // The UNSUBACK packet does not have any context specific encoding
            Self { encoded: b"\xB0\x02\x04\x07", decoded: Unsuback::new(0x0407) },
        ]
    }
}
...
impl BadEncoded {
    ...
    pub const fn all() -> &'static [Self] {
        &[
            ...
            Self { encoded: b"\xA0\x02\x04\x07" },
            // Packet with invalid length
            Self { encoded: b"\xB0\x00" },
        ]
    }
}
...
pub fn decode() {
    for test_vector in Good::all() {
        ...
        let decoded = Unsuback::try_from_iter(encoded).expect("Failed to decode valid packet");
...
pub fn decode_invalid() {
    for test_vector in BadEncoded::all() {
        // Decode and validate
        let encoded = test_vector.encoded.iter().copied();
        let decoded = Unsuback::try_from_iter(encoded);
